...
export function PaymentFormComponent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log({ amount: Number(amount), projectCode, category, description })
        ...
        <div className="space-y-2">
          ...
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              ...
              >
                {projectCode
                  ? projectCodes.find((code) => code.value === projectCode)?.label
                  : "Select project code..."}
                ...
                <CommandGroup>
                  {projectCodes.map((code) => (
                    <CommandItem
                      key={code.value}
                      onSelect={() => {
                        setProjectCode(code.value === projectCode ? "" : code.value)
                        setOpen(false)
                      }}
                    >
                      <Check
                        className={cn(
                          "mr-2 h-4 w-4",
                          projectCode === code.value ? "opacity-100" : "opacity-0"
                        )}
                      />
                      {code.label}
                    </CommandItem>
